...
static bool ifQueNeedDelete(Form_pg_attribute attrs);
static void deleteQueFromStr(char* strPara);
static void keepDigitFromStr(char* strPara);

void parserPriv(char *privStr,OperaPriv *priv);
...
void
logminer_pfree(char* ptr,int checkflag)
{
	if(ptr)
		pfree(ptr);
}
...
char*
logminer_malloc(int size,int checkflag)
{
	char *result = NULL;	
...
void
logminer_free(char* ptr,int checkflag)
{
	if(ptr)
	{
		free(ptr);
...
bool
inputParaCheck(char *st, char *et)
{
...
void
logminer_createMemContext(void)
{
...
void
logminer_switchMemContext(void)
{
...
OutputToByte(text* attrpter, int attlen)
{
	...
	{
		len = ((int)strlen((char *) (attrpter)) + 1);
		attstr = (char*)attrpter;
	}

	if(-1 ==attlen)
	{
		str = DatumGetCString(DirectFunctionCall1(byteaout, PointerGetDatum(attrpter)));
	}
	else
	{
