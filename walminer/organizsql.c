 ...
 *
 * IDENTIFICATION
 *	  contrib/xlogminer/organizsql.c
 *
 *-------------------------------------------------------------------------
 */
 ...
 *  functions prototype
 */
void parser_attribute(XLogMinerSQL *sql_opt, bool gettypname);
...
char* getGrantStrByCh(char ch, bool *wgoption);
...
void minerTbsCreate(XLogReaderState *record, XLogMinerSQL *sql_simple,uint8 info);
void minerTbsDrop(XLogReaderState *record, XLogMinerSQL *sql_simple,uint8 info);
void getfuncpara(XLogMinerSQL *sql_ori, Funct	*func);
void mentalTup_nulldata(int natts, int index, XLogMinerSQL *values_sql,XLogMinerSQL *att_sql, bool valueappend, bool attdroped, bool *firstattget);
void mentalTup_valuedata(int natts, int index, XLogMinerSQL *values_sql,XLogMinerSQL *att_sql, bool valueappend, bool attdroped, bool quoset,char* strPara, TupleDesc typeinfo,bool *firstattget);
...
void deletePgextensionSQL(XLogMinerSQL *sql_ori,XLogMinerSQL *sql_opt);
...
getInsertSQL(XLogMinerSQL *sql_simple, char *tupleInfo, NameData *relname, char* schname, bool sysrel)
{
	...
	else
	{
		...
		appendtoSQL(sql_simple, " VALUES(NULL) (NOTICE:Xlog is not enought.)", PG_LOGMINER_SQLPARA_SIMPLE);
	}
...
getDeleteSQL(XLogMinerSQL *sql_simple, char *tupleInfo, NameData *relname, char* schname, bool sysrel, bool undo)
{
	...
	{
		rrctl.prostatu = LOGMINER_PROSTATUE_INSERT_MISSING_TUPLEINFO;
		appendtoSQL(sql_simple, " WHERE VALUES(NULL) (NOTICE:Xlog is not enought.)", PG_LOGMINER_SQLPARA_SIMPLE);
	}
...
getUpdateSQL(XLogMinerSQL *sql_simple, char *tupleInfo, char *tupleInfo_old,NameData *relname, char* schname, bool sysrel)
{
	...
	else
	{
		...
		appendtoSQL(sql_simple, " SET VALUES(NULL) (NOTICE:Xlog is not enought.)", PG_LOGMINER_SQLPARA_SIMPLE);
	}
...
reAssembleUpdateSql(XLogMinerSQL *sql_ori, bool undo)
{
	...
	*/
	...
	for (i = 0; i < natts; ++i)
	{
		...
		if(nulls[i] != nulls_old[i] || 0 != strcmp(strPara ,strPara_old))
		{
			if(getchange)
				appendtoSQL_doubquo(sql_ori, ", ", false);
	...
	for (i = 0; i < natts; ++i)
	{
		...
		strPara_old = old_strPara_arr[i];
		if(!strPara_old && !nulls_old[i])
			/*should not happen*/
			ereport(ERROR,(errmsg("There are some wrong data in record.")));
		if(0 == count_value)
		{
