...
static void buildrelmap(void);
static void loadSingrelmap(FILE *fp, RelMapFile *map);
...
int
getDatadictionaryLoadType(void)
{
	PgDataDic *pdd = NULL;
	if(!DataDictionaryCache)
...
checkXlogFileRepeat(char *path,XlogFile **xfrepeat)
{
	...
	while(xfptr)
	{
		...
		XLogFromFileName(filename, &timelinecur, &segnocur, rrctl.WalSegSz);
...
buildrelmap(void)
{
	...
	if(sizeof(RelMapFile) != fwrite(&shared_map, 1, sizeof(RelMapFile), fp))
	{
		ereport(ERROR,
				(errcode(ERRCODE_SYSTEM_ERROR),
				errmsg("Fail to write to %s",dictionary_path)));
	}
...
static void
loadSingrelmap(FILE *fp, RelMapFile *map)
{
	pg_crc32c	crc;
...
static void
loadrelmap(FILE *fp,PgDataDic* pdd)
{
	Assert(fp);
	loadSingrelmap(fp, &pdd->shared_map);
	loadSingrelmap(fp, &pdd->local_map);
}
...
Oid
getRelidbyRelnodeViaMap(Oid filenode, bool shared)
{
...
outputSysTableDictionary(char *path, SysClassLevel *scl, bool self)
{
	...
	if(!XLogIsNeeded())
		ereport(ERROR,(errmsg("wal level can not support wal analyse.")));
	paddingscl(scl);
	...
	if(PG_LOGMINER_DICTIONARY_PATHCHECK_NULL == pathkind)
	{
		...
		snprintf(dictionary_path, MAXPGPATH, "%s/%s",
					 DataDir, PG_LOGMINER_PATH);
		if(!create_dir(dictionary_path))
			ereport(ERROR,(errmsg("It is failed to create dictionary \"%s\".",dictionary_path)));

		memset(dictionary_path,0,MAXPGPATH);
		snprintf(dictionary_path, MAXPGPATH, "%s/%s/%s",DataDir, PG_LOGMINER_PATH,LOGMINERDIR_DIC);
		if(!create_dir(dictionary_path))
			ereport(ERROR,(errmsg("It is failed to create dictionary \"%s\".",dictionary_path)));

		memset(dictionary_path,0,MAXPGPATH);
		snprintf(dictionary_path, MAXPGPATH, "%s/%s/temp",DataDir, PG_LOGMINER_PATH);
		if(!create_dir(dictionary_path))
			ereport(ERROR,(errmsg("It is failed to create dictionary \"%s\".",dictionary_path)));
		
		memset(dictionary_path,0,MAXPGPATH);
		snprintf(dictionary_path, MAXPGPATH, "%s/%s/%s/%s_%s",
					 DataDir, PG_LOGMINER_PATH, LOGMINERDIR_DIC, dbname_cur, PG_LOGMINER_DICTIONARY_DEFAULTNAME);
	...
	{
		snprintf(dictionary_path, MAXPGPATH, "%s/%s",DataDir, PG_LOGMINER_PATH);
		if(!create_dir(dictionary_path))
			ereport(ERROR,(errmsg("It is failed to create dictionary \"%s\".",dictionary_path)));
		memset(dictionary_path,0,MAXPGPATH);
		snprintf(dictionary_path, MAXPGPATH, "%s/%s/%s", DataDir, PG_LOGMINER_PATH, path);
	}
...
loadSystableDictionary(char *path, SysClassLevel *scl, bool self)
{
	...
	if(!create_dir(temp_path))
		ereport(ERROR,(errmsg("It is failed to create dictionary \"%s\".",temp_path)));
...
loadXlogfileList(void)
{
	...
	snprintf(xlogfilelist_path, MAXPGPATH, "%s/%s/%d_%s",
						 DataDir, PG_LOGMINER_PATH, MyProcPid, PG_LOGMINER_DICTIONARY_XLOGFILELIST);
...
loadDicStorePath(char *dicstorepath)
{
	...
	snprintf(dicstore_path, MAXPGPATH, "%s/%s/%d_%s",
						 DataDir, PG_LOGMINER_PATH, MyProcPid, PG_LOGMINER_DICTIONARY_STOREPATH);
...
writeDicStorePath(char* dicstorepath)
{
	...
	if(!fp)
	{
		...
		snprintf(logminer_dir, MAXPGPATH, "%s/%s",DataDir, PG_LOGMINER_PATH);
		if(!create_dir(logminer_dir))
			ereport(ERROR,(errmsg("fail to create dir")));
		fp = fopen(dicstore_path, "wb");
		if(!fp)
...
dropAnalyseFile()
{
	...
	snprintf(temp, MAXPGPATH, "%s/%s/%s",DataDir, PG_LOGMINER_PATH, LOGMINERDIR_DIC);
	if(is_dir_exist(temp))
		dropAllFileInADir(temp);
	
	memset(temp,0,MAXPGPATH);
	snprintf(temp, MAXPGPATH, "%s/%s", DataDir, PG_LOGMINER_PATH);
	if(is_dir_exist(temp))
		dropAllFileInADir(temp);
	memset(temp,0,MAXPGPATH);
}
...
getNextXlogFile(char *fctx, bool show)
{
	...
	XLogFromFileName(filename, &timeline, &segno, rrctl.WalSegSz);
...
Oid
getuserOidByUsername(char *user_name)
{
	PgDataDic			*pdd = NULL;
	Form_pg_authid		fpa = NULL;
	char				*serchPtr = NULL;
	...
	while(NULL != (serchPtr = logminer_getnext(PG_LOGMINER_IMPTSYSCLASS_PGAUTHID,scl)))
	{
		fpa = (Form_pg_authid)(serchPtr + sizeof(Oid));
		if(0 == strcmp(user_name,fpa->rolname.data))
		{
			result = *(int*)serchPtr;
			break;
		}
	}
	pdd->sdc[PG_LOGMINER_IMPTSYSCLASS_PGAUTHID].curdata = pdd->sdc[PG_LOGMINER_IMPTSYSCLASS_PGAUTHID].data;

	if(0 == result)
		ereport(NOTICE,(errmsg("Did not find username \"%s\" from datadictionary",user_name)));
	return result;
}
...
bool
getLangNameByOid(Oid langoid, NameData* langname)
{
	Form_pg_language	fpl = NULL;
	bool				result = false;

	fpl = (Form_pg_language)logminer_getFormByOid(PG_LOGMINER_IMPTSYSCLASS_PGLANGUAGE, langoid);
	if(!fpl)
		ereport(NOTICE,(errmsg("Did not find langoid %d in dictionary",langoid)));
	else
	{
		memcpy(langname,&fpl->lanname,sizeof(NameData));
	}
	return result;
}
...
bool
getTypnameBytypid(Oid typid, char* typname)
{
	Form_pg_type	fpt = NULL;

	fpt = (Form_pg_type)logminer_getFormByOid(PG_LOGMINER_IMPTSYSCLASS_PGTYPE, typid);
	if(!fpt)
		return false;
	else
	{
		if(0 != fpt->typelem && -1 == fpt->typlen)
		{
			getTypnameBytypid(fpt->typelem, typname);
		}
		else
		{
			memcpy(typname, fpt->typname.data, strlen(fpt->typname.data));
		}
	}
	return true;
}
...
bool
getRelAttNameByReloidAttid(Oid attoid, int attid, char* attname)
{
	PgDataDic			*pdd = NULL;
	...
	char 				*serchPtr = NULL;
	SysClassLevel 		*scl = NULL;
	int					search_tabid = PG_LOGMINER_IMPTSYSCLASS_PGATTRIBUTE;
	...
	while(NULL != (serchPtr = logminer_getnext(search_tabid,scl)))
	{
		...
		if (fpa->attrelid ==  attoid &&
			fpa->attnum == attid)
		{
			result = true;
			memcpy(attname, fpa->attname.data, strlen(fpa->attname.data));
			break;
		}
	}
	pdd->sdc[search_tabid].curdata = pdd->sdc[search_tabid].data;
